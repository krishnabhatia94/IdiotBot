...
myIntents.add(Discord.Intents.FLAGS.GUILDS, Discord.Intents.FLAGS.GUILD_MESSAGES,Discord.Intents.FLAGS.GUILD_VOICE_STATES);
const client = new Discord.Client({ intents: myIntents }); //3276799
const prefix = '>';

client.once('ready', () => {
    console.log('Idiot Bot is Online!');
    client.user.setPresence({ activities: [{ name: "some commands working!" }] });
});
...
client.on("messageCreate", message =>{
    ...
    // } else {
        ...
        if(cmd === 'idiot'){
            message.channel.send({content: 'You too.'});
        }
        if(cmd === 'danny'){
            let randomNumber = Math.random();
            if(randomNumber >= 0.5){
...
client.on('messageCreate', async message =>{
    ...
    if((message.content.includes("@everyone") || message.content.includes("@here")) && !message.member.permissions.has("MENTION_EVERYONE") || message.content.length >= 1950){
        ...
        function playSongFile(pathToFile){
            if(message.member.voice.channel && selfBot.voice.channel){
                if(message.member.voice.channel.id === selfBot.voice.channel.id){
                    ...
                    //const connection = vc.getVoiceConnection(message.member.voice.channel.id);
                    //message.member.voice.channel.join().then(connection =>{
                        //const player = voiceChat.AudioPlayer();
                        // const voiceConnect = voiceChat.joinVoiceChannel();
                        // voiceConnect.subscribe(player);
                        //const resource = voiceChat.createAudioResource(pathToFile)
        ...
        if(cmd === "join" || cmd === "joinvc"){
            if(message.member.voice.channel){
                if(!selfBot.voice.channel){
                    //const player = voiceChat.createAudioPlayer();
                    //const voiceConnect = voiceChat.joinVoiceChannel(message.member.voice.channel.id);
                    ////const voiceConnect = voiceChat.getVoiceConnection(message.guild.id);
                    //voiceConnect.subscribe(player);
                    //const voiceChannel = await message.member.voice.channel.join();
                    const connection = vc.joinVoiceChannel({
                        channelId: message.member.voice.channel.id,
                ...
                message.channel.send("I AM ALREADY IN THE VOICE CHANNEL.");
                }
            } else {
                message.channel.send(">join is used to get me into a voice channel. To get me in one, you might want to try joining one first, you half brained mortal.")
            }
        ...
        if(cmd === 'pin'){
            ...
            if(message.reference && pinSize < 50 &&message.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)){
                message.channel.messages.fetch(message.reference.messageID).then(messageFetched => messageFetched.pin());
            } else if(!message.reference && message.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) {
            ...
            } else if(pinSize >= 50 && message.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)){
        ...
        if(cmd === 'pincount' || cmd === 'pinned' || cmd === 'pinc'){
            let pinSize = (await message.channel.messages.fetchPinned()).size;
            if(pinSize == 1){
                message.channel.send(`This channel, **#${message.channel.name.toString()}**, has **${pinSize}** pinned message!`);
            } else {
                message.channel.send(`This channel, **#${message.channel.name.toString()}**, has **${pinSize}** pinned messages!`);
            }
        }
        if(cmd === 'poopy'){
...
client.login('NzcxMTk1MzY2NjQ1MDM5MTE1.GHW9rC.0B6CiqdxpzPtRcUIbq0BM1tvanlRTm0yV_dhB8');